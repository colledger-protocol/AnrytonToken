...
contract Stake is Sales, ReentrancyGuardUpgradeable {
    struct Staking {
        uint232 stakeAmount;
...
function initializeStake(address _token,address _owner) external initializer {
    __ReentrancyGuard_init();
    initializeSales(_token, _owner);
    ...
    function vesting(address user) public view returns (uint256) {
        ...
        uint depositLength = users[user].totalDeposits;
        for (uint256 m = 7; m <= 42; m++) {
            uint256 withdrawnAmt = 0;

            for (uint256 i = 1; i <= depositLength; i++) {
